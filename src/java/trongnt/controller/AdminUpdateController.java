...
public class AdminUpdateController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {            
            ...
            BookDTO book=new BookDTO(bookID, bookName, image, author, description, price, categoryID, dateAdded, quantity, Boolean.parseBoolean(status));
            if(BookDAO.updateBook(book)>0){               
                List<BookDTO> list=BookDAO.getAdminLoadListBook();
                session.setAttribute("LIST_BOOKS", list);
                url=SUCCESS;
            }        
        } catch (Exception e) {
             Logger.getLogger(AdminUpdateController.class).error("error at AdminUpdateController");
        }finally{
